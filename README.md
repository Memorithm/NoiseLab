...
For the information axis, NoiseLab adopts a simple rule: **characterize before filtering**. Raw observations must be preserved, and filtering is treated as an intervention whose information loss or gain is measured rather than assumed. See `docs/research/noise-as-information.md`. The current information diagnostics now include histogram MI, deterministic permutation, cyclic-shift and spectrum-matched nulls, a non-circular lagged-association scan that keeps one frozen observation quantization across declared lags, a known-answer conditional-MI calibration primitive for declared discrete conditioners, a Stage 0.5 frequency-selective filter retention audit that freezes equal-width bin edges from the raw observation before applying preregistered SciRust FIR/Butterworth interventions, a Stage 0.6 residual-state known-answer panel that wires oscillator/bistable/FHN/laser residuals to histogram MI with injected-bit and shuffled-label controls (exploratory calibration only; not natural-system discovery), and a Stage 0.7 attention/FLAT known-answer panel that derives per-query output/LSE residual features from the real FLAT RoPE oracle under frozen Q/K/V-site perturbations (Value-site is the explicit KV-analogue; true KV-cache deferred; synthetic calibration only, not production-model claims). Lag, conditional and filter-retention scans remain association diagnostics and do not establish causal direction or mechanism.
