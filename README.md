...
For the information axis, NoiseLab adopts a simple rule: **characterize before filtering**. Raw observations must be preserved, and filtering is treated as an intervention whose information loss or gain is measured rather than assumed. See `docs/research/noise-as-information.md`. The current information diagnostics now include histogram MI, deterministic permutation, cyclic-shift and spectrum-matched nulls, plus a non-circular lagged-association scan that keeps one frozen observation quantization across declared lags. Lag scans remain descriptive and do not establish causal direction.
